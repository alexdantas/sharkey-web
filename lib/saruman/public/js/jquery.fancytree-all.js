...
//		node.debug("ftnode.nodeKeydown(" + event.type + "): ftnode:" + this + ", charCode:" + event.charCode + ", keyCode: " + event.keyCode + ", which: " + event.which);
		...
		switch( event.which ) {
			...
			/* THIS IS MY OWN CUSTOM EVENT I MADE FOR USERS TO BE ABLE TO PRESS
			   ENTER AND SPACE AND ACTIVATE SOMTHING */

			    tree._triggerNodeEvent("link", ctx.node, ctx.originalEvent);
...
// Append a counter badge
		if( (count || ! extOpts.hideZeros) && (!node.isExpanded() || !extOpts.hideExpanded) ){
			 ...
			 *  CHANGED THIS BEIB
			 */
			$(node.span).append($("<span class='fancytree-childcounter badge pull-right'/>").text(count));
		}
	},
